...
int Indenter::indentDiff(const QString& previousLine, const TextEditor::TabSettings& tabSettings) const
{
    Scanner scanner(previousLine);
    int parens = 0;
    Token lastToken;
    ...
    if (parens != 0 && (lastToken.kind == Token::RPAREN
                        || lastToken.kind == Token::RBRACK
